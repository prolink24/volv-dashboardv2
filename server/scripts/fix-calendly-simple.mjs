/**
 * Simple Calendly Dashboard Fix
 * 
 * This script directly imports missing Calendly events for the last 30 days
 * and clears the dashboard cache so the correct call counts appear.
 */
...
async function fixCalendlyDashboard() {
  console.log('Starting simple Calendly dashboard fix...');
  ...
  try {
    ...
    for (const event of missingEvents) {
      try {
        ...
        // Insert meeting into database using only available columns from schema
        await pool.query(`
          INSERT INTO meetings (
            calendly_event_id, type, title, start_time, end_time, duration, 
            status, booked_at, assigned_to, contact_id, 
            invitee_email, invitee_name, sequence
